...
security() {
  ...
  sed -i '17s/22/63122/' $ssh_config
 #sed -i '115s/#//' $ssh_config
  sed -i '132s/yes/no/' $ssh_config
 #sed -i '38s/#//' $ssh_config
 #sed -i '38s/yes/no/' $ssh_config
  ...
  useradd qdao
  echo "NmYyNmM2YzgyMWRl#" |passwd --stdin qdao
  chown -R qdao.qdao /finance
  chown -R qdao.qdao /home/qiandao
...
firewall-cmd --zone=public --add-port=63122/tcp --permanent
